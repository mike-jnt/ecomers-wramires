const Orders = window.Orders = {
  ...
  init() {
    ...
    ["productSelect","quantityInput","shippingInput"].forEach(id => {
  ...
  updateSummary() {
    ...
    document.getElementById("productPreview").innerHTML = `
      <div class="product-preview-grid">
        <div><b>${p.name}</b><span class="product-meta">${p.variant} · ${p.sku}</span></div>
        <div class="${ownPossible?'stock-good':'stock-warn'}">${ownPossible?`${available} disponibles en bodega`:`Proveedor: ${p.supplierStock} reportados`}</div>
      </div>`;
    ...
    document.getElementById("fulfillmentCallout").innerHTML = ownPossible
      ? `<b>🏠 Inventario propio</b><span>Al confirmar se reservarán ${qty} unidad${qty>1?'es':''} mediante una transacción de Firestore.</span>`
  ...
  async create(e) {
    ...
    try {
      ...
      document.getElementById("shippingInput").value=15000;
      this.updateSummary();

      App.toast(`${result.id} creado correctamente.`);
