const Pricing = window.Pricing = {
  ...
  init() {
    ...
    this.initDone = true;
    document.getElementById("loadStartechCatalogBtn")?.addEventListener("click", () => this.loadStartechCatalog());
  },
  ...
  async loadStartechCatalog() {
    if (App.state.role !== "admin") return;
    if (!confirm("Se cargarán los 8 productos encontrados en PRECIOS STARTECH con stock 0. Los SKU existentes no se duplicarán. ¿Continuar?")) return;
    try {
      const result = await FirebaseService.seedStartechCatalog();
      App.toast(`Catálogo Startech: ${result.created} creado(s), ${result.skipped} existente(s).`);
    } catch (error) {
      console.error(error);
  ...
  render() {
    ...
    if (!active.length) {
      table.innerHTML = `<div class="empty-state"><b>No hay productos</b><span>${App.state.role === "admin" ? "Usa Cargar catálogo Startech o crea productos desde Inventario." : "El administrador todavía no ha configurado el catálogo."}</span></div>`;
      return;
    }
