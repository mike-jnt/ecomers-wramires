(() => {
  const App = window.App = {
    ...
    async seedDemo() {
      ...
      if (!confirm("Esto cargará el catálogo real de PRECIOS STARTECH con stock 0 y reiniciará/creará el contador si hace falta. Los SKU existentes no se duplicarán. ¿Continuar?")) return;
      try {
        const result = await FirebaseService.seedStartechCatalog();
        this.toast(`Catálogo Startech: ${result.created} producto(s) creado(s), ${result.skipped} existente(s).`);
      } catch (error) {
        console.error(error);
    ...
    renderDashboard() {
      ...
      if (this.state.role === "admin" && this.products.length === 0) {
        quickHtml = `<button class="quick-action" data-action="seed-demo"><b>⚙ Cargar catálogo Startech</b><span>Crear productos reales con stock 0</span></button>` + quickHtml;
      }
